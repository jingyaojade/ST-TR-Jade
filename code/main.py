...
class Processor():
    ...
    def load_checkpoint(self, path, filename):
        ...
        checkpoint = torch.load(ckpt_path, map_location = torch.device("cuda:0" if torch.cuda.is_available() and use_gpu else "cpu"))
    ...
    def adjust_learning_rate(self, epoch):
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'Adam' or self.arg.optimizer == 'Adamod':
            ...
            lr = self.arg.base_lr * (
                        self.arg.base_lr ** np.sum(epoch >= np.array(step)))  # ** 乘方
    ...
    def train(self, epoch, save_model=True):
        ...
        if (not arg.accumulating_gradients):  #通过累计梯度来解决本地显存不足问题，通过减小batch size，累计几次梯度下降之后再进行更新，再进行backward
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            ...
            self.test(
                epoch=self.epoch, save_score=self.arg.save_score, loader_name=['test'])
            self.print_log('Done.\n')
